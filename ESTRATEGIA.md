...
8. (feito 2026-07-03) Preparar adaptativo pos-composite de 04/07:
   `src/v341_360_post_july4_followups.py` gera o primario `plans/2026-07-05.csv` em
   `v341`-`v360` quando `v301`-`v320` estiverem pontuados. Ele exige composite
   public-neutral/positivo contra `v296` e cria ablacões controladas de mediastino,
   `v185` e buckets ASSOC/DIFF; se nao houver sinal publico seguro, falha com
   `not_ready` para usar a contingencia `v361`-`v380`.
9. (feito 2026-07-03) Preparar paraquedas de 06/07 sem ocupar a faixa adaptativa:
   `src/v401_420_july6_contingency.py` gera `plans/2026-07-06-public-contingency.csv`
...
11. (feito 2026-07-03) Preparar paraquedas de 08/07 sem ocupar a faixa adaptativa:
   `src/v481_500_july8_contingency.py` gera `plans/2026-07-08-public-contingency.csv`
   em `v481`-`v500`, preservando `v461`-`v480` para o adaptativo primario pos-score de
   07/07. O lote explora KEEP em condicoes public-neutral (HF, thyroid, ILD,
   Dermatomycosis, Bronchitis, NPC) sobre `v296`, com e sem mediastino `v300`.
12. (feito 2026-07-03) Preparar paraquedas de 09/07 sem ocupar a faixa adaptativa:
   `src/v521_540_july9_assoc_isolations.py` gera
   `plans/2026-07-09-public-contingency.csv` em `v521`-`v540`, preservando `v501`-`v520`
   para o adaptativo primario pos-score de 08/07. O lote isola ASSOC-only por condicao
   sobre `v296` para decompor o ganho de `v283`/`v286` sem misturar DIFF.
13. (feito 2026-07-03) Preparar paraquedas de 10/07 sem ocupar a faixa adaptativa:
   `src/v561_580_july10_diff_isolations.py` gera
   `plans/2026-07-10-public-contingency.csv` em `v561`-`v580`, preservando `v541`-`v560`
   para o adaptativo primario pos-score de 09/07. O lote isola DIFF-only por condicao
   sobre `v296`; como DIFF amplo derrubou publico, usar estes scores para confirmar se
   existe alguma excecao individual ou se DIFF deve seguir bloqueado.
14. (feito 2026-07-03) Preparar paraquedas de 11/07 sem ocupar a faixa adaptativa:
   `src/v601_620_july11_keep_prunes.py` gera
   `plans/2026-07-11-public-contingency.csv` em `v601`-`v620`, preservando `v581`-`v600`
   para o adaptativo primario pos-score de 10/07. O lote testa podas KEEP de familias
   suspeitas em condicoes privadas, mantendo COPD/Mediastinum intocados.
15. (feito 2026-07-03) Preparar paraquedas de 12/07 sem ocupar a faixa adaptativa:
   `src/v641_660_july12_prune_assoc.py` gera
   `plans/2026-07-12-public-contingency.csv` em `v641`-`v660`, preservando `v621`-`v640`
...
16. (feito 2026-07-03) Preparar paraquedas de 13/07 sem ocupar a faixa adaptativa:
   ...
   publico.
17. (feito 2026-07-03) Preparar paraquedas de 14/07 sem ocupar a faixa adaptativa:
   `src/v721_740_july14_prune_assocdiff.py` gera
   `plans/2026-07-14-public-contingency.csv` em `v721`-`v740`, preservando `v701`-`v720`
   para o adaptativo primario pos-score de 13/07. O lote combina poda KEEP + ASSOC+DIFF
   por condicao sobre `v296`; e o fallback mais agressivo da familia prune+bucket.
18. Rodar Fase B (familias KEEP) em paralelo nas balas restantes; usar o scorer para
   checar a granularidade das raizes KEEP curadas (Aortic/Stroke tem raizes nao-cheias).
19. (feito 2026-07-02) Reescrever `reports/final-candidates.md` com selecao diversificada
...
20. (feito 2026-07-02) Endurecer a operacao de cota apos duplicatas no historico Kaggle:
