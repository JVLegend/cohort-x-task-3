"""V50 — Gemini 2.5 Pro classifier. TOP_N=500, chunks=50, sleep adaptativo."""
import os
import json
import time
import requests
...
- KEEP: code IS the condition OR any subtype/variant (different anatomic sites, severities, complications WITH the condition like "diabetes WITH nephropathy", congenital/neonatal/iatrogenic/obstetric forms of the SAME disease, all specific subcodes of the condition's category).
...
Output a JSON array, one object per code: [{{"code":"X","label":"KEEP"}},{{"code":"Y","label":"IRRELEVANT"}}]. ONLY the JSON array, no prose."""
...
def call_gemini(prompt, retries=8):
    for attempt in range(retries):
        try:
            ...
            if not parts:
                # response cut by thinking tokens; retry with bigger budget? skip
                print(f"  empty parts, finish={cand.get('finishReason')}")
                return ""
            return parts[0].get("text", "")
        except Exception as e:
            if attempt < retries - 1:
                print(f"  err {attempt}: {str(e)[:100]}")
                time.sleep(5)
            else:
                raise
