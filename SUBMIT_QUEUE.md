...
- Guarda de notebooks novos: apos gerar `reports/YYYY-MM-DD-intel.md`, o `daily-run` agora para antes do `preflight`/submissao se `New public notebooks` for maior que zero, imprimindo `new_public_notebooks_guard=N` e o comando `.venv/bin/python src/sync_public_notebooks.py`; baixar, diffar e auditar o notebook antes de seguir. `--allow-new-notebooks` e override manual depois dessa auditoria, nao rotina da automacao.
