...
- `final-candidates` tambem pode ser executado isoladamente para consolidar a shortlist de selecao final: recomenda ate 20 arquivos em `reports/final-candidates.md`, escreve o espelho operacional `reports/final-selection.csv`, preserva uma ancora publica/inalterada, suplementa anchors historicos conhecidos (`v178`, `v185`) quando a listagem recente da Kaggle os omite, promove ASSOC/DIFF near-best como hedge estrategico, aceita hedges ate `0.00325` abaixo do melhor publico, preenche slots restantes com reservas publicas controladas ate `0.00600` abaixo do melhor e filtra mutacoes KEEP-only grandes demais da selecao recomendada.
