...
1. Revalidar Kaggle:
   ...
   - `.venv/bin/python src/cohortx_ops.py deadline-readiness`
...
9. Atualizar:
   ...
   - `reports/YYYY-MM-DD-decision-outcome.md` com `.venv/bin/python src/cohortx_ops.py plan-decision-outcome plans/YYYY-MM-DD.csv`.
...
Se esse `daily-run` for executado antes de 2026-07-06 em UTC com `--date 2026-07-06`, ele valida o plano e gera `intel`/plan-report, mas imprime `target_date_relation=future`, `date_guard=skip_submit` e `post_reports_guard=no_current_plan_activity` sem chamar a submissao nem atualizar review/signals/scorecard/final-candidates.
...
.venv/bin/python src/cohortx_ops.py plan-decision-outcome plans/2026-07-04.csv --anchor submissions/v296_copd_no_j20_j45_j81_j82_j93_j95.csv
...
Equivalente historico do plano primario de 2026-07-05:
...
Equivalente expandido para o proximo plano primario de 2026-07-06:

```bash
.venv/bin/python src/cohortx_ops.py validate-plan plans/2026-07-06.csv
.venv/bin/python src/cohortx_ops.py preflight --date 2026-07-06
.venv/bin/python src/cohortx_ops.py plan-report plans/2026-07-06.csv
.venv/bin/python src/cohortx_ops.py plan-strategy plans/2026-07-06.csv
.venv/bin/python src/cohortx_ops.py plan-manifest plans/2026-07-06.csv
.venv/bin/python src/cohortx_ops.py plan-decision plans/2026-07-06.csv
.venv/bin/python src/audit_plan_deltas.py --plan plans/2026-07-06.csv --out reports/2026-07-06-code-deltas.md
.venv/bin/python src/cohortx_ops.py daily-run --auto-next-plan
```
...
- `plan-manifest` gera `reports/YYYY-MM-DD-manifest.md`, registrando SHA-256, 23 linhas esperadas, volume de mudanca, condicoes alteradas e eixos estrategicos por CSV; o `daily-run` tambem gera esse manifesto automaticamente para o plano selecionado e para planos adaptativos novos; `preflight` e `readiness` comparam esses hashes com os CSVs atuais e marcam `manifest=drift` se algum arquivo mudou depois do manifesto;
- `plan-decision-outcome` gera `reports/YYYY-MM-DD-decision-outcome.md` depois dos scores, transformando as comparacoes pareadas em vencedores/empates por eixo, mostrando volume por variante, recomendando menor volume em empates publicos e marcando `pending_scores` quando a Kaggle ainda nao pontuou o suficiente;
...
- `daily-run` encadeia status, intel pre-submissao, preflight, validacao, plan-report, plan-strategy, plan-manifest, plan-decision, deltas de plano, submissao, review, signals, plan-scorecard, plan-decision-outcome e final-candidates, mas tambem respeita `recommended_action` do preflight antes de chamar `submit_plan`, imprimindo `preflight_guard=...` para `wait_for_quota`/bloqueios, e bloqueia submissao quando a data alvo for futura/passada, o deadline ja tiver passado ou o intel detectar notebook publico novo/atualizado ainda nao baixado/auditado;
...
- o gerador moderno `src/v341_360_post_july4_followups.py` aplica penalidade de prioridade em candidatos com volume de mudanca acima de 1000 contra `v296`, para que planos futuros `v381+` nao sejam dominados por hedges amplos quando houver opcoes de score parecido e menor raio de impacto;
...
- `plan-decision-outcome` deve ser lido junto com o scorecard: ele evita promover um arquivo isolado quando uma comparacao pareada por eixo mostra que o ganho veio de outro fator; em empate publico, seguir a coluna `Recommended` antes de ampliar hedges privados.
...
.venv/bin/python src/cohortx_ops.py plan-decision-outcome plans/2026-07-03.csv
```
